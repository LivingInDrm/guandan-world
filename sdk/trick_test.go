...
func TestTrickGetTrickSummary(t *testing.T) {
	trick, _ := NewTrick(1)
	trick.StartTrick()
	...
	cards = append(cards, card)
	comp := NewSingle(cards)
	trick.PlayCards(1, cards, comp)
	
	summary := trick.GetTrickSummary()
	
	if summary == nil {
		t.Fatal("GetTrickSummary should return a non-nil summary")
	}
	
	if summary["id"] != trick.ID {
		t.Error("Summary should include trick ID")
	}
	
	if summary["leader"] != 1 {
		t.Error("Summary should include correct leader")
	}
	
	if summary["play_count"] != 1 {
		t.Error("Summary should include correct play count")
	}
	
	plays, ok := summary["plays"].([]map[string]interface{})
	if !ok || len(plays) != 1 {
		t.Error("Summary should include plays array")
	}
	
	if plays[0]["player_seat"] != 1 {
		t.Error("Play summary should include correct player seat")
	}
}