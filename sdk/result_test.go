...
func TestDealResultCalculator_CalculateDealResult(t *testing.T) {
	...
	deal.Rankings = []int{0, 2, 1, 3} // Team 0 (seats 0,2) wins
	now := time.Now()
	deal.EndTime = &now
...
func TestDealResultCalculator_VictoryTypes(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:        "Normal Victory",
			rankings:    []int{0, 1, 2, 3}, // Alternating teams
			expectedType: VictoryTypeNormal,
			expectedUpgrades: [2]int{1, 0},
		},
		{
			name:        "Double Down Victory",
...
func TestDealResult_HelperMethods(t *testing.T) {
	...
	result := &DealResult{
		Rankings:    []int{0, 2, 1, 3}, // Team 0 wins (double down)
		WinningTeam: 0,
	...
	// Team 0 should have ranks 1 and 2
	expectedTeam0Ranks := []int{1, 2}
	...
	if !result.IsDoubleDown() {
		t.Error("Expected IsDoubleDown to return true")
	}
	
	if result.IsTripleDown() {
		t.Error("Expected IsTripleDown to return false")
	}
