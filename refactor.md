...
```typescript
import { GamePhase, PHASE_TRANSITIONS } from '../types';
...
export class GameStateManager {
  ...
  // 强制同步（极端兜底）
  forceSync(newPhase: GamePhase, reason: string): void {
    this.clearPhaseTimeout();
    this.currentPhase = newPhase;
    this.phaseStartTime = Date.now();
    this.setupPhaseTimeout(newPhase);
    console.warn(`[StateManager] forceSync -> ${newPhase} (${reason})`);
  }
  ...
  const queryGameState = async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        // 强制同步到后端状态
        stateManager.current.forceSync(phase, 'state sync');
...
func (h *GameDriverHandler) GetGameState(c *gin.Context) {
    ...
    full := c.DefaultQuery("full", "0") == "1"  // 是否返回完整视图
    
    // 短缓存（200ms 内复用，削峰）
    if resp, ok := h.stateCache.GetRecent(roomID, 200*time.Millisecond, full, userID); ok {
    ...
    case room.RoomStatusPlaying:
        ...
        if err != nil {
            ...
            if engine != nil {
                ...
                if full {
                    playerView = convertPlayerViewToMap(view) // 完整视图
                }
            } else {
                phase = "game_preparing"
            }
...
useEffect(() => {
  ...
  const pollInterval = setInterval(() => {
    scheduleQueryGameState();  // 每 10 秒轮询一次
  }, 10000);
...
useEffect(() => {
  if (isConnected) {
    scheduleQueryGameState();  // 连接成功后立即同步状态
  }
}, [isConnected]);
...
func (h *RoomHandler) runGamePrepareSequence(roomID string, players [4]*room.Player) {
    ...
    // 启动游戏引擎（首批 player_view 由引擎 ready/首个事件触发的 observer 同步发送）
...
func (wso *WebSocketObserver) OnGameEvent(event *sdk.GameEvent) {
    ...
    case wso.eventQueue <- event:
        // 事件已入队
    default:
        if wso.isDroppable(event.Type) {
            log.Printf("Info: queue full, drop mergeable event: %s", event.Type)
        } else {
            // 关键事件不可丢弃，退化为同步处理
...
// 合并/可丢弃事件判断（心跳、进度等）
func (wso *WebSocketObserver) isDroppable(t sdk.GameEventType) bool {
    switch t {
    case sdk.GameEventTypeHeartbeat, sdk.GameEventTypeProgress:
        return true
    default:
        return false
    }
...
export class MessageQueue {
  ...
  private lastSeq: number = -1; // 按房间单调递增的序列
  private maxQueueSize = 200;
  
  enqueue(message: WSMessage): void {
    const seq = this.getSeq(message);
    if (seq <= this.lastSeq) {
      // 去重：旧消息或重复消息
      return;
    }
  ...
  processMessages(handler: (message: WSMessage) => void): void {
    // 基于 seq 排序，严格递增处理
    this.queue.sort((a, b) => this.getSeq(a.message) - this.getSeq(b.message));
    for (const item of this.queue) {
      const seq = this.getSeq(item.message);
      if (seq > this.lastSeq) {
        handler(item.message);
        this.lastSeq = seq;
      }
    }
    // 仅保留尚未处理且大于 lastSeq 的消息
    this.queue = this.queue.filter(q => this.getSeq(q.message) > this.lastSeq);
  }
  
  private getSeq(message: WSMessage): number {
    // 兼容：优先 top-level seq，其次 data.seq
    // @ts-ignore
    return (message as any).seq ?? (message.data?.seq ?? -1);
  }
...
所有服务器下行 WS 消息携带 `seq`（按房间单调递增），用于排序与去重。
